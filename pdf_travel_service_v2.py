...
# PDF Hunting 
file_names = []
def pdf_hunting(s_path, d_path, extension):
    present_extension = {"PDF":".pdf", "Excel":[".xlsx", ".xls", ".xl", ".xlsm", ".xlsb", ".xlw"], "CSV":".csv"}[str(extension)]
    n_files = 0
    for root, dirs, files in os.walk(s_path):
        for file in files:
            if(str(file).endswith(tuple(present_extension))):
